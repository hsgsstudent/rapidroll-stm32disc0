...
/**
 * @brief  Reads the button states and returns them as a bitmask.
 * @retval uint8_t Bitmask (UP=0x01, DOWN=0x02, LEFT=0x04, RIGHT=0x08)
 */
uint8_t HAL_Get_Button_State(void) {
  uint8_t state = 0;

  if (HAL_GPIO_ReadPin(BTN_PORT, BTN_UP_PIN) == GPIO_PIN_SET)
    state |= MSK_UP;
  if (HAL_GPIO_ReadPin(BTN_PORT, BTN_DOWN_PIN) == GPIO_PIN_SET)
    state |= MSK_DOWN;
  if (HAL_GPIO_ReadPin(BTN_PORT, BTN_LEFT_PIN) == GPIO_PIN_SET)
    state |= MSK_LEFT;
  if (HAL_GPIO_ReadPin(BTN_PORT, BTN_RIGHT_PIN) == GPIO_PIN_SET)
    state |= MSK_RIGHT;
